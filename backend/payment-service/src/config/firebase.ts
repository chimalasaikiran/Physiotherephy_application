...
const initFirebaseAdmin = (): any => {
  ...
  for (const filePath of candidatePaths) {
    try {
      if (fs.existsSync(filePath)) {
        ...
        console.log(`✅ Loaded Firebase Service Account in payment-service from: ${filePath}`);
  ...
  console.log(`ℹ️ Initializing payment-service Firebase Admin with Project ID: ${projectId}`);
  return admin.initializeApp({
    projectId,
  });
};
